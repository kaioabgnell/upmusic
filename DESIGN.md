...
- Signature Mintlify mint green ({colors.brand-orange}) reserved for accent CTAs, active states, and feature confirmations
...
### Brand & Accent
- **Mintlify Mint** ({colors.brand-orange}): Signature accent — used on hero "Get started" pill button, green checkmarks in feature lists, featured pricing tier border accent, sidebar active indicator dots.
- **Deep Mint** ({colors.brand-orange-deep}): Pressed/active variant of the mint accent.
- **Soft Mint** ({colors.brand-orange-soft}): Subtle background tint for success states and confirmation surfaces.
...
- Background `{colors.brand-orange}`, text `{colors.primary}`, typography `{typography.button-md}`, padding `10px 20px`, rounded `{rounded.full}`.
...
- Background `{colors.canvas}`, rounded `{rounded.lg}`, padding `{spacing.xxl}`, border `2px solid {colors.brand-orange}`, soft brand-tinted shadow `rgba(0, 212, 164, 0.08) 0px 8px 24px`.
...
- Border switches to `2px solid {colors.brand-orange}` — focus uses the brand mint as the activation signal.
...
- Background `{colors.brand-orange}`, text `{colors.primary}`, typography `{typography.caption-bold}`, rounded `{rounded.full}`, padding `2px 8px`.
...
- Active: text `{colors.ink}`, typography `{typography.body-sm-medium}`, optional left-border accent in `{colors.brand-orange}`.
...
### Do
- Reserve `{colors.brand-orange}` (Mintlify mint) for accent CTAs and active state indicators only — even one accent button per viewport carries weight
...
### Don't
- Don't use `{colors.brand-orange}` on body text or large surfaces — it loses signal
...
6. Keep `{colors.brand-orange}` confined to accent moments. If it appears on a generic surface, ask whether it earned that role.
