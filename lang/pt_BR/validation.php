...
return [
    ...
    |--------------------------------------------------------------------------
    |
    | Traduções em português (Brasil) para as regras de validação padrão do
    | Laravel. Regras/mensagens específicas (CPF, CNPJ, etc.) ficam nas
    | próprias classes de Rule (app/Rules) via $fail().
    |
    */
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | Fallback para campos que não definem :attribute explicitamente no Form
    | Request (via attributes()). A maioria dos módulos já traduz por lá.
    |
    */
